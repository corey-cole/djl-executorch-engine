...
Supported platforms: `linux-x86_64`, `linux-aarch64` and `windows-x86_64`. **All three ship the `devtools` runtime variant** — the event tracer behind opt-in profiling — so `EtEngine.devtoolsAvailable()` answers `true` everywhere as of pin `1.4.1-3`. It is still the contract, never the platform name: the list lives in `native/variant_select.sh` (`ET_DEVTOOLS_SUPPORTED_PLATFORMS`) and a platform can leave it, and an explicit `ET_RUNTIME_VARIANT=logging` build answers `false` on an otherwise provisioned platform. `bare` remains a Linux-only benchmarking build, and `ET_RUNTIME_VARIANT` still overrides the list for benchmarking.
...
- The runtime's first-party custom op `etnp::lstm` is whole-archived into the shim when the tarball
  ships `lib/cmake/ETNPExtras/ETNPExtras.cmake` (auto-detected in `native/CMakeLists.txt`). **Both
  Linux tarballs ship it**, x86_64 and aarch64 — aarch64 gained it when the platform moved to
  `devtools`, verified at pin `1.4.1-3`. The Windows tarball does not, which is the whole reason
  `LstmModelIT` is gated at all. That gate tests the OS rather than querying the op, which is the
  wrong shape by this file's own rule; issue #64 tracks exposing custom-op registration the way
  `EtNative.backendRegistered` exposes backends.
...
`-DCMAKE_BUILD_TYPE=Release` is passed on every platform, for two independent reasons. On Windows it's an ABI constraint: MSVC encodes the CRT flavour into every object and refuses to mix them, and the pinned runtime tarball is built Release (`/MD`), so a non-Release shim fails to link with `LNK2038` `RuntimeLibrary`/`_ITERATOR_DEBUG_LEVEL` mismatches. GCC/ELF has no such ABI tag, but leaving `CMAKE_BUILD_TYPE` unset there is not a no-op either: with none given, CMake adds no `-O` flag and the compiler's own default is `-O0` — every Linux/macOS shim build was unoptimized until this was fixed, **including the artifact CI stages into the released JAR**. Measured end-to-end through `EtSymbolBlock` on a 41-input model: ~104.9us at the old (accidental) `-O0` vs ~39.9us at `-O3`, roughly 2.6x. Output is `executorch_djl.dll` (no `lib` prefix), staged into `src/main/resources/native/windows-x86_64/`.
...
- `Criteria.optOption(EtEngine.PROFILING_OPTION, "true")` attaches ExecuTorch's event tracer to **one model** at load; the ETDump grows across every forward until pulled via `((EtModel) zooModel.getWrappedModel()).etDump()`. There is deliberately **no JVM property** — a property would let one flag enable unbounded event accumulation for every model in the process without touching code, so enabling is a decision at the load site. `EtEngine.devtoolsAvailable()` is the capability contract — all three shipped platforms are provisioned, but a `logging` build answers `false`, so never branch on the platform name. Requesting profiling without the capability fails the load. The ETDump buffer is released through `flatcc_builder_aligned_free`, not `free()`: it is an aligned allocation, and only the Windows arm can prove the difference, since on glibc both expand to `free()`. See `docs/profiling.md`.
