...
**Native layer** (`native/`)
- `core/et_runtime.{h,cpp}` — a **JNIEnv-free** C++ core (`measly::et::EtRuntime`) that wraps the ExecuTorch `Module`. Deliberately free of any JVM dependency so it can be linked by the shim, the Catch2 unit tests, and the leak harness alike. Borrowed input pointers, single-copy out into a JNI-allocated direct buffer, freed by a `java.lang.ref.Cleaner` once the Java buffer is unreachable. **The input borrow is not zero-copy end to end:** ExecuTorch's `Method::set_input` copies into its own arena for any input where `is_memory_planned()` is true — the export default, and true for every `.pte` in this repo — and honors the borrow only for models exported with `alloc_graph_input=False`. See `docs/executorch-host-buffer-contract-brief.md`.
